...
class PeakFit(object):
	...
	def fit(self):
		...
		for n,g in enumerate(self.gm):
			...
			bounds[0] += [0.0, idx-B_f*sig*0.5, sig-sig*np.exp(-0.1*B_f)]
...
class Spectrum(object):
	...
	def _default_params(self):
		...
		self._meta = {'fit_config':{'snip_adj':1.0, 'R':0.1, 'alpha':0.9, 
									...
									'fit_bound':2.5, 'xrays':False, 'pk_width':7.5, 
									'E_min':75.0, 'I_min':0.05}, 'istp':[], 'shelf':None}
		self.cb = Calibration()
	...
	def snip_bg(self):
		...
		x, dead = np.arange(len(self._hist)), int(7.5*adj*self.cb.res(len(self._hist)))
	...
	def gamma_list(self):
		if self._gamma_list is None:
			...
			self._gamma_list = [(np.array([i['E'], i['I'], i['dI']]).T)*np.array([1.0, 0.01, 0.01]) for n,i in enumerate(gammas)]
	...
	def plot(self, fit=True, labels=True, square_fig=False, snip=False, **kwargs):
		...
		if snip:
			ax.plot(self.cb.eng(range(len(self._snip))), self._snip)
